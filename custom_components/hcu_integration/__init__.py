...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Register an update listener to reload the integration when options change (e.g., for lock PIN).
    ...
    def _handle_event_message(message: dict):
        ...
        if updated_ids:
            # Broadcast a signal to entities that their underlying data has been updated.
    ...
    # It is automatically cancelled by Home Assistant when the entry is unloaded.
    ...
    try:
        # Fetch the initial state. The listener is now ready to receive the response.
