...
class HcuApiClient:
    ...
    async def async_set_dim_level(self, device_id: str, channel_index: int, dim_level: float, ramp_time: float | None = None) -> None:
        ...
        api_path_key = "SET_DIM_LEVEL"
        if ramp_time is not None:
            body["rampTime"] = ramp_time
            api_path_key += "_WITH_TIME"
        await self.async_device_control(API_PATHS[api_path_key], device_id, channel_index, body)
