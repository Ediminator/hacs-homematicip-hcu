            ...
            if channel_mapping := HMIP_CHANNEL_TYPE_TO_ENTITY.get(channel_type):
                ...
                if module := class_module_map.get(class_name):
                    try:
                        ...
                        init_kwargs = {"config_entry": config_entry} if channel_type == "DOOR_LOCK_CHANNEL" else {}
                        entities[platform].append(
                            entity_class(coordinator, client, device_data, channel_index, **init_kwargs)
                        )
                    except (AttributeError, TypeError) as e:
                        _LOGGER.error("Failed to create entity for channel %s (%s): %s", channel_type, class_name, e)
